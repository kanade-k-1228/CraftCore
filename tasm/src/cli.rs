...
use crate::collect_module_files;

#[derive(Debug, clap::Parser)]
#[clap(author, version, about)]
pub struct Args {
    /// Input files (default: `tasm.srcs` of project.yaml, or main.tasm)
    pub src: Vec<String>,

    /// Output binary file
    #[clap(short, long, default_value = "out/main.bin", value_name = "FILE")]
    pub out: String,

    /// Output rom file
    #[clap(short, long, default_value = "out/const.bin", value_name = "FILE")]
    pub rom: String,

    /// Generate map file
    #[clap(short = 'm', long = "map", num_args = 0..=1, default_missing_value = "out/map.yaml", value_name = "FILE")]
    pub map: Option<String>,

    /// Module include directories (repeatable). `NAME=DIR` maps module NAME to
    /// DIR (`<DIR>/a/b.tasm` is module `NAME::a::b`); a bare `DIR` uses its
    /// basename as the module root name.
    #[clap(short = 'I', long = "include", value_name = "[NAME=]DIR")]
...
pub fn run(args: &Args) -> Result<(), crate::Error> {
    ...
    let sources = {
        ...
        // Module files from `-I [NAME=]DIR`.
        let mut module_files: Vec<(String, String)> = vec![];
        for inc in &args.include {
            let (root, dir) = crate::parse_include_spec(inc);
            collect_module_files(Path::new(&dir), &root, &mut module_files)?;
        }
    ...
    let tokens = {
        ...
        for (path, text) in sources.iter() {
            let toks = crate::Lexer::new(path, text).parse();
            tokens.extend(toks);
        }
        tokens
    };
    ...
    let (mut ast, errors) = crate::Parser::new(tokens.into_iter()).parse();
    if !errors.is_empty() {
        for e in &errors {
            eprintln!("error: {}", e);
        }
    ...
    crate::apply_module_prefixes(&mut ast, &modmap);

    // 4. Evaluator Database
    let global = crate::Global::new(&ast, modmap)?;
    ...
    let mut ialoc = crate::Memory::new(0, 0x10000)
    ...
    let mut daloc = crate::Memory::new(0, 0x10000)
    ...
    // 7. Generate binary
    if args.verbose {
        crate::binprint(&imap, &dmap, &global);
    }
    let main_bin = crate::genibin(&global, &imap, &dmap)?;
    let const_bin = crate::gencbin(&global, &dmap)?;
    let symbol_map = crate::SymbolMap::generate(&global, &imap, &dmap);

    // 9. Write output files
    for path in [&args.out, &args.rom].into_iter().chain(args.map.as_ref()) {
